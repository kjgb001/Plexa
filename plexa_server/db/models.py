...
class LessonRecord(Base):
    ...
    __table_args__ = (
        UniqueConstraint("owning_course_id", "lesson_id", "version", name="uq_course_lesson_version"),
    )

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    owning_course_id: Mapped[int] = mapped_column(
    ...
    artifact_revision: Mapped[int] = mapped_column(Integer, default=1, nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utc_now, nullable=False)
...
class SessionRecord(Base):
    ...
    lesson_snapshot: Mapped[dict[str, Any] | None] = mapped_column(JSONB, nullable=True)
    lesson_artifact_revision: Mapped[int] = mapped_column(Integer, default=1, nullable=False)
    lesson_content_sha256: Mapped[str | None] = mapped_column(String(64), nullable=True)
    transcript_available: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    transcript_unavailable_reason: Mapped[str | None] = mapped_column(String(64), nullable=True)
    persistence_revision: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
...
class MessageRecord(Base):
    ...
    __table_args__ = (
        UniqueConstraint("session_id", "sequence_index", name="uq_session_message_index"),
        UniqueConstraint("session_id", "message_id", name="uq_session_message_id"),
    )
...
class EncryptedLogRecord(Base):
    ...
    content_available: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
